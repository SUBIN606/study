...
---
- 람다 표현식이란?
  - 수학에서는 람다대수라고 해서 임의의 값? 추상 기호로 쓰이긴 한다.
  - 익명의 어떤 것..을 할 때 람다로 썼다.
  - 메서드: 특정 클래스에 종속됨
  - 함수: 특정 클래스에 종속되지 않음 -> 정적 메서드는 함수다. 클래스에 종속되지 않고 사용할 수 있기 때문에
- 람다에서의 시그니처는 자바에서의 메서드 시그니처가 다르다..
- 함수형 인터페이스
  - 단 하나의 추상 메서드만 갖는 인터페이스
  - 디폴트 메서드가 있더라도 추상 메서드가 오직 하나면 함수형 인터페이스다
- 함수형 인터페이스로 만들면 뭐가 좋은가?
  - 람다표현식을 쓸 수 있음
- 함수 디스크립터
  - 람다 표현식의 시그니처를 서술하는 메서드
  - 얘가 어떤 동작을 하는데 이게 어떤 함수형 인터페이스와 맞아 그래서 쓸 수 있다
  - 함수형 인터페이스의 추상메서드 시그니처
- 실행 어라운드 패턴
  - 예제 코드 보면 바꿀 수 있는 동작만 람다로 전달해서 유연하게 만들 수 있다.
- 제네릭에서 기본형을 사용할 수 없는 이유
  - 기본형은 Object를 상속받지 않아 변환이 불가능하기 때문
- 함수형 인터페이스는 확인된 예외를 던지는 동작을 허용하지 않는데 왜 그런걸까
  - 자바에서 제공하는 함수형 인터페이스들은 확인된 예외를 던지는 동작이 없으니 네가 알아서 해라
- 대상형식
  - 대상형식 덕분에 같은 람다 표현식으로 다양한 함수형 인터페이스에 사용할 수 있다
  - 동일한 함수 디스크립터를 가지면 대상형식이 같다
- 람다에서 사용하는 지역변수
  - 람다는 익명클래스의 인스턴스 -> 지역변수를 인스턴스의 변수로 복사함 -> 그래서 힙에 저장됨
- 익명클래스의 변수는 쉐도잉되는데 람다는 안된다.
  - 람다는 외부 범위의 변수를 캡처할 때 읽기전용으로 취급하여 변수를 복사하지 않고 직접 참조
  - 익명 클래스는 외부 범위의 변수를 캡처할 때 해상 변수를 복사해서 사용한다 그래서 익명 클래스에서 외부 변수를 수정하거나 변경한느 경우 원래 범위의 변수에 영향을 주지 않는다
  - 변수를 복사하는 것을 쉐도잉이라 한다.
